...
```
Job (oportunidade)
  └── JobEngagement (por usuário)
        isFavorite, priority, visibility, hiddenAt

Application (acompanhamento seletivo)
  stage, lastStageUpdatedAt, notes, timeline, status
        └── TimelineEvent[]
```

Atributos de engajamento e estágio do pipeline são **independentes** (ADR-022). Detalhes: [API_CONTRACT.md](./API_CONTRACT.md#domínio--enums-oficiais).
...
- **Mutations MVP:** favoritar, definir prioridade, ocultar/restaurar, marcar visualizada, **abrir vaga** (`sourceUrl` na plataforma original)
- **Campos de engajamento:** `isFavorite`, `priority`, `visibility`, `hiddenAt` (contratos Etapa 10.7; UI Etapa 12)
...
O Pipeline **não representa a candidatura** — apenas o acompanhamento manual da jornada do usuário.

- **Contrato público:** `GET /pipeline`, `PATCH /pipeline/:id/status|favorite|notes`, `POST /pipeline`, `DELETE /pipeline/:id`, `GET /pipeline/:id/timeline`
- **Campos:** `lastStageUpdatedAt` atualizado automaticamente em movimentações; timeline com eventos automáticos
- **Estágio `"favorite"` legado:** remoção planejada na Etapa 12
...
Ver lista completa em [MVP_SCOPE.md](./MVP_SCOPE.md).

- **Aplicação pela plataforma** — candidatura sempre na origem
- **Upload de currículo / carta de apresentação**
- **Perfil público, LinkedIn/GitHub/portfólio no perfil**
- **Providers reais, Scheduler, WebSocket, IA, Analytics, ML, i18n**
- **Redis** — cache no cliente via React Query
