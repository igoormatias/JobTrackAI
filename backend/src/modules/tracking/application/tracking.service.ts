...
import {
  ...
  type InMemoryJobTrackingRepository,
...
export class TrackingService {
  ...
  constructor(repository?: TrackingRepository) {
    this.repository =
      repository ?? (isTestEnv ? inMemoryJobTrackingRepository : prismaJobTrackingRepository);
  }
  ...
  async getById(id: string): Promise<JobTrackingEntity> {
    const tracking = await this.repository.findById(id);
  ...
  async moveStage(id: string, input: MoveTrackingStageInput): Promise<JobTrackingEntity> {
    return this.repository.moveStage(id, input);
  }
  ...
  async changePriority(id: string, priority: JobPriority): Promise<JobTrackingEntity> {
    return this.repository.changePriority(id, priority);
  }
  ...
  async updateNotes(id: string, notes: string | null): Promise<JobTrackingEntity> {
    return this.repository.updateNotes(id, notes);
  }
  ...
  async updateTimelineEvent(
    ...
    return this.repository.updateTimelineEvent(trackingId, eventId, input);
  }

  async getTimeline(id: string): Promise<TrackingTimelineEvent[]> {
